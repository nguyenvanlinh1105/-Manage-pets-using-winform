...
('DV011', N'Tiếp sức sinh lý', 180000, N'VitaPlus', N'Thực phẩm', 20, 10, N'Hộp', '2024-04-06 08:00:00.000', NULL, NULL, NULL),
...
('DV014', N'Đặt đơn giao hàng', 120000, N'Fast Delivery', N'Dịch vụ giao hàng', 0, 10, N'Lần', '2024-04-06 11:00:00.000', NULL, NULL, NULL),
('DV015', N'Khử mùi chó mèo', 95000, N'FreshPets', N'Chăm sóc thú cưng', 30, 15, N'Lọ', '2024-04-06 12:00:00.000', NULL, NULL, NULL),
('DV016', N'Chăm sóc sức khỏe cho mèo', 180000, N'Pet Health', N'Chăm sóc thú cưng', 0, 5, N'Lần', '2024-04-07 08:00:00.000', NULL, NULL, NULL),
('DV017', N'Tắm gội cho mèo', 120000, N'Pet Spa', N'Chăm sóc thú cưng', 0, 8, N'Lần', '2024-04-07 09:00:00.000', NULL, NULL, NULL),
('DV018', N'Đặt khay vệ sinh cho mèo', 85000, N'Cat Litter', N'Chăm sóc thú cưng', 0, 12, N'Lần', '2024-04-07 10:00:00.000', NULL, NULL, NULL),
...
('DV028', N'Thải độc cho mèo', 100000, N'Pet Detox', N'Chăm sóc thú cưng', 0, 9, N'Lần', '2024-04-08 11:00:00.000', NULL, NULL, NULL),
...
values	('PN08','NCC09', '2023-10-21 08:00:00', NULL, 'AD001', NULL),
		...
		update PhieuNhap
		set createdby = N' Lưu Ngọc Yến Như'
...
select maDonHang, ChiTietDonHang.idDichVu as idDichVu, tenDichVu, BS.idUser + ' - ' + BS.tenuser as tenBS,NV.idUser + ' - ' + NV.tenuser as tenNV, soLuong,DichVu.donGia, ChiTietDonHang.createddate,ChiTietDonHang.createdby
from ChiTietDonHang 
...
create or alter view dbo.PN as
select  maPN ,NV.maNCC + ' - ' + NV.tenNCC as NCC,  PhieuNhap.createddate,PhieuNhap.createdby
from PhieuNhap 
left join NhaCungCap NV on PhieuNhap.maNCC = NV.maNCC
...
CREATE or alter PROCEDURE UpdateCTDT
     @maDH varchar(50),
	@idDV nvarchar(50),
@idBS nvarchar(50),
@idNV nvarchar(50),
...
BEGIN
	update ChiTietDonHang set
	 idBacSi = @idBS, idNhanVien =@idNV, soLuong =@sl, createddate = @createddate, createdby = createdby
	where maDonHang = @maDH and idDichVu = @idDV
END;

create or alter view dbo.chitietPN as
select  maPN ,NV.idDichVu + ' - ' + NV.tenDichVu as DichVu, soLuongNhap, giaNhap,  ChiTietPhieuNhap.createddate,ChiTietPhieuNhap.createdby
from ChiTietPhieuNhap 
left join DichVu NV on ChiTietPhieuNhap.idDichVu = NV.idDichVu
select * from dbo.chitietPN

CREATE or alter PROCEDURE InsertCTPN
    @maPN varchar(50),
	@idDV nvarchar(50),
@gia money,
@sl int,
...
AS
BEGIN
    INSERT INTO ChiTietPhieuNhap(maPN, idDichVu, giaNhap, soLuongNhap, createddate, createdby)
    VALUES (@maPN, @idDV, @gia, @sl, @createddate, @createdby);
END;


CREATE or alter PROCEDURE UpdateCTPN
    @maPN varchar(50),
	@idDV nvarchar(50),
@gia money,
@sl int,
...
BEGIN
 ...
 giaNhap =@gia, soLuongNhap =@sl, createddate = @createddate, createdby = @createdby
	where maPN = @maPN and idDichVu = @idDV
END;
...
exec InsertKhachHang
'hoa111','123',N'LeHoa',N'fgf',N'Nam','2024-01-02','0125476983','pgnhet4@gmail.com','2024-01-02 00:00:00.000', N'nguyet',0
...
CREATE or alter FUNCTION HamGetNextMaPhieuNhap()
RETURNS varchar(10)
AS
BEGIN
...
CREATE  or alter FUNCTION HamGetNextIdDichVu()
RETURNS varchar(10)
AS
BEGIN
    ...
    SET @NewIdDichVu = 'DV' + RIGHT('00' + CONVERT(varchar(10), @NextId), 2);
...
BEGIN
    ...
    INSERT INTO DichVu (idDichVu, tenDichVu, donGia, thuongHieu, loaiSanPham,soLuongDaBan, soLuongHienCon, donViTinh, createddate, createdby)
    VALUES (@NewIdDich, @tenDichVu, @donGia, @thuongHieu, @loaiSanPham,0,0, @donViTinh, @createddate, @createdby);
END;

-- proc update dichVu
CREATE or alter PROCEDURE UpdateDichVu
    @idDichVu varchar(10),
    @tenDichVu nvarchar(100),
    @donGia float,
    @thuongHieu nvarchar(100),
...
BEGIN
    ...
    INNER JOIN DichVu DV ON CHD.idDichVu = DV.idDichVu
    INNER JOIN DonHang DH ON CHD.maDonHang = DH.maDonHang
    WHERE DH.idKhachHang = @idKhachHang and DH.trangThai in ( N'Chưa thanh toán')
	ORDER BY CHD.maDonHang desc;
END;
select * from dbo.DonHang


execute GetInvoiceInfoChuaThanhToan 'KH002'
...
-- proc lấy những đơn hàng của khách hàng trong giỏ hàng
...
execute InsertIntoThe 'TC035', 'KH034', '12-05-2024', N'Nguyễn Văn Linh'
...
SELECT 
    SUM(CASE WHEN dv.donViTinh = N'Lần' THEN ctdh.SoLuong * dv.DonGia ELSE 0 END) AS ThanhTien_Lan,
    SUM(CASE WHEN dv.donViTinh <> N'Lần' THEN ctdh.SoLuong * dv.DonGia ELSE 0 END) AS ThanhTien_Khac_Lan,
...
ORDER BY 
	...
	INSERT INTO Users (idUser, userName, password, tenuser, loaiuser, gioiTinh, ngaySinh, soDienThoai, email, thongTinUser, createddate, createdby, modifieddate, modifiedby, status)
VALUES
('AD111', 'admin123', '123', N'Phạm Thị Thu Nguyệt', N'Người quản trị hệ thống', N'Nữ', '1975-03-07', '0542379147', 'admin1234@gmail.com', NULL, '2024-04-06 10:00:00.000', 'ADMIN', NULL, NULL, 1)
